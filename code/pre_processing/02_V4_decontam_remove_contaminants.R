# Remove likely contaminants from 16S amplicon data (contaminants in the
...
# found in script 'V4_DADA2_generate_ASV_table.R'

# Will use the decontam package (https://github.com/benjjneb/decontam)

# Had to update bioconductor to 3.10 from 3.9 (only need to do this once)
...
# Install decontam (only need to do this once)
...
metadata <- read.table("data/raw/malawi_tilapia_metadata.txt", header = TRUE,
                       row.names = 1)
seqtab <- readRDS("data/process/V4_ASVtable_final.rds")
tax <- readRDS("data/process/V4_tax_bayesian_final.rds")
...
water.pruned # 10195 taxa and 74 samples
...
water.pruned2 # 10195 taxa and 74 samples
...
swab # 11189 taxa and 58 samples
swab.pruned # 7361 taxa and 58 samples
...
swab.pruned2 # 7361 taxa and 58 samples
...
table(water.cont$contaminant) # 62
table(swab.cont$contaminant) # 78

# Give the rank of the first few most abundant contaminants
head(which(water.cont$contaminant)) # 5, 10, 11, 18, 27, 292
head(which(swab.cont$contaminant)) # 5, 10, 18, 23, 27, 39



# Frequency of contaminant ASVs against DNA concentration
plot_frequency(water.pruned2, taxa_names(water.pruned2)[which(water.cont$contaminant)[1:16]], conc="Conc_16S") + 
  xlab("DNA Concentration (ng/uL)") + ggtitle("Contaminant ASVs in water samples")


plot_frequency(swab.pruned2, taxa_names(swab.pruned2)[which(swab.cont$contaminant)[1:16]], conc="Conc_16S") + 
...
water.nocontam # 10133 taxa and 74 samples

swab.nocontam <- prune_taxa(!swab.cont$contaminant, swab.pruned2)
swab.nocontam # 7283 taxa and 58 samples
...
ps.nocontam # 11106 taxa and 126 samples

# Write to disk
saveRDS(ps.nocontam, "data/process/V4_phyloseq_obj_after_decontam.rds")
...
ps.no.controls # 11106 taxa and 116 samples

# Remove ASVs that are unclassified at the Domain level or that classify as euk
ps.domain <- subset_taxa(ps.no.controls, Kingdom != "Eukaryota" & !is.na(Kingdom))
ps.domain # 10877 taxa and 116 samples

# Remove mitochondria and chloroplasts
# subset_taxa defaults to removing NAs too, so need to specify that they be kept
ps.no.mito <- subset_taxa(ps.domain, (Family != "Mitochondria") | is.na(Family))
ps.no.mito # 10850 taxa and 116 samples
ps.no.organelles <- subset_taxa(ps.no.mito, (Order != "Chloroplast") | is.na(Order))
ps.no.organelles # 10605 taxa and 116 samples

# Save phyloseq object before filtering out small libraries and caudal/gill swabs
saveRDS(ps.no.organelles, "data/process/V4_phyloseq_obj_final.rds")

# Remove samples with fewer than 1500 reads (suggesting failed library prep)
ps1 <- prune_samples(sample_sums(ps.no.organelles) >= 1500, ps.no.organelles)

# Remove caudal and gill swabs, and any ASVs with fewer than 5 counts in total
ps2 <- ps1 %>% subset_samples(!(sample_type_detailed %in% c("Swab_cd", "Swab_gl")))
ps.no.organelles2 <- prune_taxa(taxa_sums(ps2) > 4, ps2)
...
ps.no.organelles2 # 9145 taxa and 95 samples


# Change ASV names ####
# With the combined phyloseq object, change the ASV names from the full 253bp
...
# Save this object prior to phylogenetic tree step in script V4_generate_tree.R
saveRDS(ps.no.organelles2, "data/process/V4_phyloseq_obj_after_organelle_removal.rds")

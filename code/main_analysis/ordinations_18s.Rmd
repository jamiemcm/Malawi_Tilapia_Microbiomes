---
title: '18S biplots with marginal density plots and boxplots'
...
Load the 18S (V9)  phyloseq object, subset samples, remove ASVs with fewer than 10 reads overall, and extract the various data tables (since I'm not going to use phyloseq beyond this step).  Removing the rare ASVs reduces the zero overinflation, though keeping these rare ASVs doesn't have much effect on the results. This appears particuarly the case for this Malawi dataset, and major site trends still hold true.
```{r load data PL tank plus direct inputs}
V9.tree <- readRDS(here("data", "process", "V9_phyloseq_obj_with_tree_prev_filt.rds")) #note this 18S phyloseq object has had the top ASV attributed to tilapia removed) 
V9.tree #working with 1659 taxa by 63 samples

V9.sub <- subset_samples(V9.tree, sample_type %in% "Pond") %>% tidyPS(., FALSE)
sample_data(V9.sub)$Pond <- as.factor(sample_data(V9.sub)$Pond)
V9.sub #working with 1650 taxa by 35 samples

V9.pond <- V9.sub
#previously (in 16S) we have removed asvs with fewer than 10 reads, to reduce zero overinflation, in this data set though it  does reduce the number of taxa by 400, although it makes no difference to the ordinations. Downstream no change to conclusions.
#V9.pond <- prune_taxa(taxa_sums(V9.sub) > 10, V9.sub)
V9.pond ##working with 1650 taxa by 35 samples

##check all computes remain the same! *****

asvs_V9 <- as.data.frame(otu_table(V9.pond))
asvs_V9 <- as_tibble(asvs_V9, rownames = "Sample_ID", colnames = "ASV_ID")

taxs_V9 <- as.data.frame(tax_table(V9.pond))
taxs_V9 <- as_tibble(taxs_V9, rownames = "ASV_ID")

samples_V9 <- as.data.frame(sample_data(V9.pond))
samples_V9 <- as_tibble(samples_V9, rownames = "SampleID")

# Make a streamlined metadata object, relastically we are only concerned by pond site in this case
sample_vars_V9 <- samples_V9 %>% select(SampleID, Pond, sample_type)
```
...
```{r V9 biplot tank inputs, include =TRUE}
V9_czm <- zCompositions::cmultRepl(otu_table(V9.pond), method = "CZM") #zero replacment method, czm is  more roboust to zeros than gbm
...
V9_comp <- CoDaSeq::codaSeq.clr(V9_czm) 

# Run the PCA on the CLR-transformed data
V9_PCA <- prcomp(V9_comp)

# Visualize eigenvalues/variances
fviz_screeplot(V9_PCA, addlabels = TRUE)


# Generate biplot with colours for pond sites
pmain_V9 <- fviz_pca_biplot(V9_PCA, 
                axes = c(1, 2), # which PCs to plot
                repel = TRUE, # avoid text overlapping
                # Settings for samples
                geom = "point", 
                col.ind = sample_vars_V9$Pond, # Grouping variable, pond site
                pointsize = 4,
  ...
  #coord_cartesian(xlim = c(-50, 50), ylim = c(-40, 40)) + # specify coordinates to align boxplots
...
## Stats testing
Statistical testing of significant difference between pond sites. We utilise two tests here, firstly. a permanova through the adonis function that is the main test for difference between groups. (Note the test take the centroid i.e. median position of each group and uses this for the test) The seperation is so obvious by eye that we don't really need to do this, but if you want a P value here it is!
...
beta_permanova(V9_comp, sample_vars_V9$Pond)
...
contrib_asv <- c("ASV4", "ASV41", "ASV47", "ASV30", "ASV29", "ASV23", "ASV8", "ASV6", "ASV85", "ASV81", "ASV74", "ASV91", "ASV16", "ASV17", "ASV26") #manual curation of a vector for top 15 asvs contributing to dissimilarity between groups on PCA.

ps_clr <- V9.pond
otu_table(ps_clr) <- otu_table(V9_comp, taxa_are_rows = FALSE) # create a phyloseq object with compositional transformed counts
...
m <- 1
  ...
  heatmap <- pheatmap::pheatmap(reorder.otu.mat,
                                ...
                                cluster_rows = TRUE,
                                cluster_cols = FALSE,
                                #labels_row = as.expression(newnames), #this created a problem with row labels incorrectly assigned to corresponding rows, soloution preassign labels to DF
                                show_rownames = TRUE,
...
ggsave(here("results", "figures", "pond_18s_ordination.svg"), plot = pmain_V9, width = 6, height = 5, dpi = 600)
...
ggsave(here("results", "figures", "contrib_heatmap_pond_18s.svg"), plot = final_heatmap, width = 8, height = 5, dpi = 600)
